...
def forward(image_number):
    ...
    if image_number<len(image_list):
        ...
        button_forward= Button(root,text='Reached the End', state=DISABLED).grid(row=1, column=0)
...
button_backward= Button(root, text='<<',padx=40, command=backward, bg='blue', fg='white').grid(row=1, column=0)
